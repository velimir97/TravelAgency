...
def next_possible_arrangements():
    ...
    next_arangements = ArangementModel.query.filter_by(start_date > datetime.new())
    my_arangements = [a.id for a in current_user.tourist_arangements]
    return jsonify([a.to_json for a in next_arangements if a not in my_arangements]), 200
